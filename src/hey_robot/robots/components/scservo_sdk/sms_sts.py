...
class sms_sts(protocol_packet_handler):
    ...
    def torque_enable(self, scs_id: int = -1):
        if scs_id == -1:
            return self.writeTxRx(BROADCAST_ID, SMS_STS_TORQUE_ENABLE, 1, [1])
        return self.writeTxRx(scs_id, SMS_STS_TORQUE_ENABLE, 1, [1])

    def torque_disable(self, scs_id: int = -1):
        if scs_id == -1:
            return self.writeTxRx(BROADCAST_ID, SMS_STS_TORQUE_ENABLE, 1, [0])
        return self.writeTxRx(scs_id, SMS_STS_TORQUE_ENABLE, 1, [0])

    def set_midpoint(self, scs_id: int = -1):
        """Force the current physical position to be treated as the midpoint (2048).

        The SMS_STS protocol uses a special write of 128 to the
        ``SMS_STS_TORQUE_ENABLE`` register to trigger the internal
        recalibration.  After this write the servo considers its present
        angle as 2048 (center) without moving.  A broadcast id writes to all
        connected servos.
        """
        if scs_id == -1:
    ...
    def SyncReadPos(self, scs_ids):
        ...
        for scs_id in scs_ids:
            ...
            #                 print("[ID:%03d] PresPos:%d PresSpd:%d" % (scs_id, scs_present_position, self.scs_tohost(scs_present_speed, 15)))
            else:
